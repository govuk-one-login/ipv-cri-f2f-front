...
When(/^I sent the request to the callback endpoint$/, { timeout: 2 * 50000 }, async function () {
  const testHarness = new TestHarness();
...
Then(/^the Verifiable Credential is stored as expected$/, { timeout: 2 * 50000 }, async function () {
  const testHarness = new TestHarness();
  let sqsMessage;
  do {
...
Then(/^all TxMA events are recorded as expected$/, { timeout: 2 * 50000 }, async function () {
  const testHarness = new TestHarness();
  let sqsMessage;
  do {
    sqsMessage = await testHarness.getSqsEventList("txma/", this.sessionId, 7);
  } while (!sqsMessage);
