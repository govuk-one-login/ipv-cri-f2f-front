...
Given(
  /^the user navigates directly to brp-invalid-future-expiry page$/,
  async function () {
...
Then(
  /^the user sees an error message displayed on the brp-invalid-future-expiry page$/,
  async function () {
...
Given(
  /^the user navigates directly to brp-invalid-past-expiry page$/,
  async function () {
...
Then(
  /^the user sees an error message displayed on the brp-invalid-past-expiry page$/,
  async function () {
    const errorPage = new ErrorPage(await this.page);
    expect(await errorPage.isCurrentPage()).to.be.true;
    const redirectionError = await errorPage.getSomethingWentWrongMessage();
...
Then(
  /^the user sees an error message displayed on the brp-valid page$/,
  async function () {
    const errorPage = new ErrorPage(await this.page);
    expect(await errorPage.isCurrentPage()).to.be.true;
    const redirectionError = await errorPage.getSomethingWentWrongMessage();
