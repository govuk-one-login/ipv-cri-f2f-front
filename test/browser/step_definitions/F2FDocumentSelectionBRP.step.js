...
Then(
  /^the user is routed to the next screen in the journey BRP Expiry Date$/,
  async function () {
...
Given(
  /^the date entered is within accepted BRP expiration window$/,
  async function () {
    const brpDetailsPage = new BRPDetailsPageValid(await this.page);
    expect(await brpDetailsPage.isCurrentPage()).to.be.true;
...
When(
  /^the user clicks the continue button on the BRP Page$/,
  async function () {
    const brpDetailsPage = new BRPDetailsPageValid(await this.page);
...
Then(
  /^the user is routed to the next screen in the BRP journey - Branch Finder$/,
  async function () {
