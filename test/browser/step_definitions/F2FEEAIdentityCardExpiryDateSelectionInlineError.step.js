...
Given(
  /^the user clicks the continue button only on the EEAIdentityCardHasExpiryDatePage$/,
  async function () {
...
Then(
  /^the user sees an inline error message displayed on the EEAIdentityCardHasExpiryDatePage$/,
  async function () {
    const eeaIdentityCardHasExpiryDatePage =
      new EeaIdentityCardHasExpiryDatePage(await this.page);
    ...
    const errorBody =
      await eeaIdentityCardHasExpiryDatePage.checkErrorBodyText();

    expect(await errorBody).to.equal(inlineErrorBody);

    const errorBodyAboveRadioOptions =
      await eeaIdentityCardHasExpiryDatePage.checkErrorAboveRadioButtonText();
