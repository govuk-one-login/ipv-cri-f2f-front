...
const { EEAIdentityCardDetailsPageValid } = require("../pages");
  ...
  Given(/^the user clicks the continue button only on the EEAPage$/, async function () {
    const eeaIdentityCardPage = new EEAIdentityCardDetailsPageValid(await this.page);
  ...
  Then(/^the user sees an inline error message displayed on the EEAPage$/, async function () {
        const eeaIdentityCardPage = new EEAIdentityCardDetailsPageValid(await this.page);
